...
// Function to load and deserialize the heap snapshot
async function parseSnapshot() {
    try {
        // Read the JSON file
        const rawData = fs.readFileSync('heapSnapshotInstagram.json', 'utf8');

        // Deserialize the JSON content
        const heapSnapshot = JSON.parse(rawData);

        const parsedSnapshot = parse(heapSnapshot)

        const serializedParsedSnapshot = JSON.stringify(parsedSnapshot); // The second parameter (null) is for replacer function, third parameter (2) is for spacing

        // Save the serialized heapSnapshot to a.json file
        await fs.promises.writeFile('parsedSnapshot.json', serializedParsedSnapshot, (err) => {
            if (err) throw err;
...
// from: https://github.com/julianburr/chrome-heap-snapshot-parser
